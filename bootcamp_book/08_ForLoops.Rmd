...
## Add base commands to your tool belt!
...
* **Note** Now that you have the skill to code your conditional statements, 
here is an example of an R package that streamlines raw syntax and logic for you - in other words..  *there's an ~~app~~ package for that!* `{-}`
...
```{r the easy way out, error = FALSE}

library(dplyr) # load the library
# install.packages('dplyr') # of you do not have the package above, remove the hashtag and Cnrtl+Enter to run this line tp install
...
# install.packages('ggplot2') # of you do not have the package above, remove the hashtag and Cnrtl+Enter to run this line tp install
...
**Objective**: create a new column `Audrey_III.Length` based on the relationships with Pedal.Length for each iris species
